...
public:
    ...
    void addAttribute(QString name, QString value) {
        m_attribute = QPair<QString, QString>(name, value);
    }
    QPair<QString, QString> attribute() const { return m_attribute; }

    void setValue(int column, QString value) {
        if (column == 0) {
            m_attribute.first = value;
        }
        if (column == 1) {
            m_attribute.second = value;
        }
    }
...
private:
    QPair<QString, QString> m_attribute{};
