...
//    qDebug() << "registerResource: " << QResource::registerResource("qml.qrc");
    ...
    qDebug() << "screens[0]: ";
    qDebug() << "screens[0]: " << screens[0];
    qDebug() << "screens[1]: " << screens[1];
    if (screens.size() > 1) {
        const QRect secondScreenGeometry = screens.at(1)->virtualGeometry();
//        m.setGeometry(secondScreenGeometry);
        m.move(secondScreenGeometry.topLeft());
    }
