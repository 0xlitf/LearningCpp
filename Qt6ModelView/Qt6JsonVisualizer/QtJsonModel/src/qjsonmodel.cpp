...
QJsonTreeItem* QJsonTreeItem::load(const QJsonValue& value, QJsonTreeItem* parent) {
    ...
    if (value.isObject()) {
        ...
        if (value.type() == QJsonValue::Type::Bool)
            summary->setBooleanCount(summary->getBooleanCount() + 1);
        else if (value.type() == QJsonValue::Type::Double)
            summary->setNumberCount(summary->getNumberCount() + 1);
        else if (value.type() == QJsonValue::Type::Null)
            summary->setNullCount(summary->getNullCount() + 1);
        else if (value.type() == QJsonValue::Type::String)
            summary->setStringCount(summary->getStringCount() + 1);
    }
